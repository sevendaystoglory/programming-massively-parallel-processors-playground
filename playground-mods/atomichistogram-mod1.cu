...
// we actually saw that in out naive atomic computation of the histogram, we had a long waitlist because multiple threads were trying to access the same bin across blocks and within a block.
// resolution phase 1: maintain a temp hist in shared memory for each of the blocks
...
__global__ void histo_kernel(unsigned char * buffer, unsigned int *hist){
        ...
        if (idx < N) {
            atomicAdd(&temp[buffer[idx]], 1);
        }
        __syncthreads();
        if (threadIdx.x < 256){atomicAdd( &(hist[threadIdx.x]), temp[threadIdx.x] );} // merge with global memory
        __syncthreads();
}
